...
private:
	...
	void Ver(pbnodo nodo); int nClaves, nodosMinimos; pbnodo Entrada;
};
btree::btree(int nClv) : nClaves(nClv) {
	lista = new stclave[nClaves + 1]; listapunt = new pbnodo[nClaves + 2]; nodosMinimos = nClaves / 2;  Entrada = NULL;
}
btree::~btree() {
	delete[] lista; delete[] listapunt; BorrarNodo(Entrada);
}
void btree::BorrarNodo(pbnodo nodo) {
	int i;
	if (!nodo) return;
	for (i = 0; i <= nodo->clavesUsadas; i++)
		BorrarNodo(nodo->puntero[i]); delete nodo;
}
void btree::Mostrar() {
	cout << "Arbol" << endl; Ver(Entrada);
	cout << "-----" << endl; system("pause");
}
void btree::Ver(pbnodo nodo) {
	int i;
	if (!nodo) return;
	for (i = 0; i < nodo->clavesUsadas - 1; i++) cout << nodo->clave[i].valor << "-"; if (nodo->clavesUsadas) cout << nodo->clave[i].valor << " [";
...
long btree::Buscar(int clave) {
	pbnodo nodo = Entrada; int i;
	while (nodo) {
		i = 0;
		while (i < nodo->clavesUsadas && (nodo->clave[i].valor < clave)) i++; if (nodo->clave[i].valor == clave) return nodo->clave[i].registro;
...
bool btree::Insertar(stclave clave) {
	...
	nodo = padre;
	Inserta(clave, nodo, NULL, NULL); return true;
}
void btree::Inserta(stclave clave, pbnodo nodo, pbnodo hijo1, pbnodo hijo2) {
	...
	do {
		if (!nodo)
		{
			nodo = new bnodo(nClaves); nodo->clavesUsadas = 0; nodo->padre = NULL; Entrada = nodo;
		}
		padre = nodo->padre;
		if (nodo->clavesUsadas == nClaves) {
			...
			while (nodo->clave[i].valor < clave.valor && i < nClaves) {
				...
				listapunt[i] = nodo->puntero[i]; i++;
			}
			lista[i] = clave; listapunt[i] = hijo1; listapunt[i + 1] = hijo2;
			while (i < nClaves) {
				lista[i + 1] = nodo->clave[i]; listapunt[i + 2] = nodo->puntero[i + 1]; i++;
			}
			nodo->clavesUsadas = nClaves / 2;
			for (j = 0; j < nodo->clavesUsadas; j++) {
				nodo->clave[j] = lista[j]; nodo->puntero[j] = listapunt[j];
			}
			...
			for (j = 0; j < nuevo->clavesUsadas; j++) {
				nuevo->clave[j] = lista[j + (nClaves / 2) + 1]; nuevo->puntero[j] = listapunt[j + (nClaves / 2) + 1];

			}
			nuevo->puntero[nuevo->clavesUsadas] = listapunt[nClaves + 1];
			for (j = 0; j <= nodo->clavesUsadas; j++)
				if (nodo->puntero[j]) (nodo->puntero[j])->padre = nodo; for (j = 0; j <= nuevo->clavesUsadas; j++)
					if (nuevo->puntero[j]) (nuevo->puntero[j])->padre = nuevo; clave = lista[nClaves / 2];
			hijo1 = nodo;
			hijo2 = nuevo;
			nodo = padre;
		}
...
void btree::BorrarClave(pbnodo nodo, int valor) {
	...
	if (nodo->puntero[0]) {
		...
		while (actual->puntero[0]) actual = actual->puntero[0]; nodo->clave[pos] = actual->clave[0];
		pos = 0;
	}
	else actual = nodo;
	for (i = pos; i < actual->clavesUsadas; i++)
		actual->clave[i] = actual->clave[i + 1]; actual->clavesUsadas--;
	...
	do {
		...
		if (derecha && derecha->clavesUsadas > nodosMinimos)
			...
			FundirNodo(izquierda, padre, actual, posClavePadre - 1); actual = padre;
...
{
	int i;
	for (i = nodo->clavesUsadas;
		i > 0; i--) nodo->clave[i] = nodo->clave[i - 1];
	for (i = nodo->clavesUsadas + 1;
		i > 0; i--) nodo->puntero[i] = nodo->puntero[i + 1];
	nodo->clavesUsadas++;
...
{
	int i;
	izquierda->clave[izquierda->clavesUsadas] = padre->clave[posClavePadre]; padre->clavesUsadas--;
	for (i = posClavePadre; i < padre->clavesUsadas; i++) {
		padre->clave[i] = padre->clave[i + 1]; padre->puntero[i + 1] = padre->puntero[i + 2];
	}
	izquierda->clavesUsadas++;
